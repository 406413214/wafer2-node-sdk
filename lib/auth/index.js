...
function authorization(req) {
    ...
    return getSessionKey(code, thirdAppId)
        .then(pkg => {
            ...
            // 存储到数据库中
            return AuthDbService.saveUserInfo(openid, skey, session_key, thirdAppId, aid).then(userinfo => ({
    ...
    if (!skey) {
        ...
        return redis.get(`u:${skey}`).then(user => {
            if (!user) {
                ...
                let userInfo = JSON.parse(user)
                ctx.state.$wxInfo = {user: userInfo}
                return next()
            }
        })
    }
