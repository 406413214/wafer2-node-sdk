                ...
                return redis.exists(`app:${aid}:usercount`).then(userCount => {
                    if (parseInt(userCount) === 0) {
                        ...
                        return mysql('user').insert({
                            open_id, skey, created_at, last_visit_time, source_aid, open_id, session_key
                        }).then(uid => {
                            if (!ownerUid) {
