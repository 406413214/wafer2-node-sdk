...
function saveUserInfo(open_id, skey, session_key, thirdAppId, aid) {
    ...
    return mysql('user').count('open_id as hasUser').where({
        ...
        if (res[0].hasUser) {
            ...
            return mysql('app').select('aid', 'type').where({
                app_id: thirdAppId
            }).then(res => {
                let aid = res[0].aid
                ...
                return redis.exists(`app:${aid}:usercount`).then(active => {
                    if (parseInt(active) === 0) {
                        return mysql('user').count('open_id as cont').where({
                            aid: aid,
                            type: 1
                        }).then(res => {
                            let type = 2;
                            if (parseInt(res[0].cont) === 0) {
                                type = 1;
                            }
