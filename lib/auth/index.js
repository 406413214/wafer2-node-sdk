...
function authorization(req) {
    const {
        ...
        'x-wx-iv': iv,
        'x-wx-appid':thirdAppId
    } = req.headers
    // 检查 headers
    if ([code, encryptedData, iv, thirdAppId].some(v => !v)) {

        debug(ERRORS.ERR_HEADER_MISSED)
        throw new Error(ERRORS.ERR_HEADER_MISSED)
    }

    debug('Auth: code: %s, encryptedData: %s, iv: %s', code, encryptedData, iv, thirdAppId)
    // 获取 session key
    return getSessionKey(code, thirdAppId)
        .then(pkg => {
            ...
            // 存储到数据库中
            return AuthDbService.saveUserInfo(decryptedData, skey, session_key, thirdAppId).then(userinfo => ({
